...
public class SpeciesGraphBuilderMapper extends MapReduceBase implements Mapper<LongWritable, Text, Text, Text> { 
	...
	public void map(LongWritable key, Text value, OutputCollector<Text, Text> output, Reporter reporter) throws IOException
	{
		...
		if (title.length() > 0) { 
			...
			//adding the title to the HashSet
			//WikiSpecies.species.add(title);
		...
		//Creating the value of the page title
		StringBuilder builder = new StringBuilder(); 
		for (String link : outlinks) { 
			link = link.replace(" ", "_");
			
			//adding the outlink to the HashSet			
			//WikiSpecies.species.add(link);
	...
	//Utility function to get the page title
